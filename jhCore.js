...
clone[p]=obj[p];return clone;};jhCore.formatDateTimeSmall=function(t){var g=new Date(),j=new Date(t*1000),d=Math.floor(g.getTime()/1000)-t;var h=j.getHours(),m=fn.fill2(j.getMinutes());var f=["CN","T2","T3","T4","T5","T6","T7"];if(d<518400){d=(g.getDay()+7-j.getDay())%7;var k="";if(d!=0){if(d==1){k=" hĂ´m qua"}else{k=f[j.getDay()]}}
return h+":"+m+k;}
h=fn.fill2(h);return h+":"+m+" "+fn.fill2(j.getDate())+"/"+fn.fill2(j.getMonth()+1)+"/"+j.getFullYear()};jhCore.formatDateTime=function(t){var e=[[11,"sĂ¡ng"],[14,"trÆ°a"],[19,"chiá»u"]],f=["Chá»§ Nháº­t","Thá»© Hai","Thá»© Ba","Thá»© TÆ°","Thá»© NÄƒm","Thá»© SĂ¡u","Thá»© Báº£y"],g=new Date(),j=new Date(t*1000),d=Math.floor(g.getTime()/1000)-t;if(d<60){return(d<0?0:d).toString()+" giĂ¢y trÆ°á»›c"}
if(d<3600){return Math.floor(d/60)+" phĂºt trÆ°á»›c"}
if(d<43200){return Math.floor(d/3600)+" tiáº¿ng trÆ°á»›c"}
var h=j.getHours(),m=fn.fill2(j.getMinutes());if(d<518400){var b="tá»‘i";for(var i=0;i<3;i++){if(h<e[i][0]){b=e[i][1];break}}
d=(g.getDay()+7-j.getDay())%7;var k="";if(d==0){k="hĂ´m nay"}else{if(d==1){k="hĂ´m qua"}else{k=f[j.getDay()]}}
...
return fullname;var ll=fullname.length;return fullname.substring(li+1,ll);};jhCore.wgTime=function(delta){if(!delta)delta=0;jh('.stime').each(function(){var rel=parseInt(this.getAttribute('rel'));this.innerHTML=jhCore.formatDateTime(rel,delta);});};jhCore.repMessage=function(txt){txt=txt.replace(/@(\w+)(\:|\s|\,|\.|$)/ig,' <a href="/$1" class="display" rel="$1"></a>$2 ');txt=txt.replace(/(^|\s|;)((http|https):\/\/.*?)(\s|$|;)/ig,' <a target=\"_blank\" href="$2">$2</a> ');return txt;};jhCore.noUnicode=function(text){text=text.replace(/(Ă¡|Ă |áº£|Ă£|áº¡|Ă¢|áº¥|áº§|áº©|áº«|áº­|Äƒ|áº¯|áº±|áº³|áºµ|áº·)/ig,'a');text=text.replace(/(Ă|Ă€|áº¢|Ăƒ|áº |Ă‚|áº¤|áº¦|áº¨|áºª|áº¬|Ä‚|áº®|áº°|áº²|áº´|áº¶)/ig,'A');text=text.replace(/(Ă³|Ă²|á»|Ăµ|á»|Ă´|á»‘|á»“|á»•|á»—|á»™|Æ¡|á»›|á»|á»Ÿ|á»¡|á»£)/ig,'o');text=text.replace(/(Ă“|Ă’|á»|Ă•|á»Œ|Ă”|á»|á»’|á»”|á»–|á»˜|Æ |á»|á»œ|á»|á» |á»¢)/ig,'O');text=text.replace(/(Ă©|Ă¨|áº»|áº½|áº¹|Ăª|áº¿|á»|á»ƒ|á»…|á»‡)/ig,'e');text=text.replace(/(Ă‰|Ăˆ|áºº|áº¼|áº¸|Ă|áº¾|á»€|á»‚|á»„|á»†)/ig,'E');text=text.replace(/(Ăº|Ă¹|á»§|Å©|á»¥|Æ°|á»©|á»«|á»­|á»¯|á»±)/ig,'u');text=text.replace(/(Ă|Ă™|á»¦|Å¨|á»¤|Æ¯|á»¨|á»ª|á»¬|á»®|á»°)/ig,'U');text=text.replace(/(Ă­|Ă¬|á»‰|Ä©|á»‹)/ig,'i');text=text.replace(/(Ă|ĂŒ|á»ˆ|Ä¨|á»)/ig,'I');text=text.replace(/(Ă½|á»³|á»·|á»¹|á»µ)/ig,'i');text=text.replace(/(Ă|á»²|á»¶|á»¸|á»´)/ig,'I');text=text.replace(/(Ä‘)/ig,'d');text=text.replace(/(Ä)/ig,'D');return text;};var ua=navigator.userAgent.toLowerCase(),m=ua.match(/(^|\s)(firefox|safari|opera|msie|chrome)[\/:\s]([\d\.]+)/)||["","","0.0"];jhCore.browser={opera:m[2]=="opera",msie:m[2]=="msie",safari:m[2]=="safari",chrome:m[2]=="chrome",firefox:m[2]=="firefox",mozilla:/mozilla/.test(ua),version:(ua.match(/\sversion\/([\d\.]+)/)||["",m[3]])[1]};jhCore.storage={put:function(key,item,exp,force){key="jhvar-"+key;if(exp==undefined){exp=0}
...
return f.join("&")}});jhCore.postFormJson=function(objID,onSucess){return jh("#"+objID).postForm(onSucess);};var ViString=(function(){var c=[["Ă¡","Ă ","áº£","Ă£","áº¡","Ă¢","áº¥","áº§","áº©","áº«","áº­","Äƒ","áº¯","áº±","áº³","áºµ","áº·"],["Ă","Ă€","áº¢","Ăƒ","áº ","Ă‚","áº¤","áº¦","áº¨","áºª","áº¬","Ä‚","áº®","áº°","áº²","áº´","áº¶"],["Ă©","Ă¨","áº»","áº½","áº¹","Ăª","áº¿","á»","á»ƒ","á»…","á»‡"],["Ă‰","Ăˆ","áºº","áº¼","áº¸","Ă","áº¾","á»€","á»‚","á»„","á»†"],["Ăº","Ă¹","á»§","Å©","á»¥","Æ°","á»©","á»«","á»­","á»¯","á»±"],["Ă","Ă™","á»¦","Å¨","á»¤","Æ¯","á»¨","á»ª","á»¬","á»®","á»°"],["Ă­","Ă¬","á»‰","Ä©","á»‹"],["Ă","ĂŒ","á»ˆ","Ä¨","á»"],["Ă½","á»³","á»·","á»¹","á»µ"],["Ă","á»²","á»¶","á»¸","á»´"],["Ă³","Ă²","á»","Ăµ","á»","Ă´","á»‘","á»“","á»•","á»—","á»™","Æ¡","á»›","á»","á»Ÿ","á»¡","á»£"],["Ă“","Ă’","á»","Ă•","á»Œ","Ă”","á»","á»’","á»”","á»–","á»˜","Æ ","á»","á»œ","á»","á» ","á»¢"],["Ä‘"],["Ä"]],d=["a","A","e","E","u","U","i","I","y","Y","o","O","d","D"];function b(g){if(typeof g=="string"){for(var f=0;f<c.length;f++){for(var e=0;e<c[f].length;e++){g=g.replace(new RegExp(c[f][e],"g"),d[f])}}}
return g}
