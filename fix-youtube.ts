...
(async () => {
    try {
        ...
        if (docSnap.exists()) {
            ...
            if (config.socials) {
                config.socials.youtube = "@acxuantai";
                config.socials.youtubeName = "A.C Xuân Tài";
                config.socials.youtubeFollowers = 35400;
                await setDoc(docRef, config, { merge: true });
                console.log("Successfully updated Firestore config for youtube to @acxuantai.");
            }
        }
    } catch(e: any) {
